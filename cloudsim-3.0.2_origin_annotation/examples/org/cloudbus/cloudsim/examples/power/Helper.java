...
public class Helper {
	 ...
	 * @return the list< vm>
	 */
	public static List<Vm> createVmList(int brokerId, int vmsNumber/* cloudletList.size()*/) {
		...
		for (int i = 0; i < vmsNumber; i++) {
			...
			vms.add(new PowerVm(
					...
					1,
					"Xen",
					new CloudletSchedulerDynamicWorkload(Constants.VM_MIPS[vmType], Constants.VM_PES[vmType]),//创建云任务调度动态负载对象。
					Constants.SCHEDULING_INTERVAL/*调度间隔*/));
		}
		return vms;
	}
	...
	public static List<PowerHost> createHostList(int hostsNumber) {
		...
		for (int i = 0; i < hostsNumber; i++) {
			int hostType = i % Constants.HOST_TYPES;//取模，给主机均匀分配不同的配置。
