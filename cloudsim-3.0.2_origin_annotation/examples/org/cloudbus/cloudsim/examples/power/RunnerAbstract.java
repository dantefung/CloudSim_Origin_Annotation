...
public abstract class RunnerAbstract {
	...
	public RunnerAbstract(
		...
		init(inputFolder + "/" + workload);//workload/planetlab/888
		start(
				getExperimentName(workload, vmAllocationPolicy, vmSelectionPolicy, parameter),/*888_iqr_mmt_1.5*/
				outputFolder,//output
	...
	protected void initLogOutput(
		...
		setEnableOutput(enableOutput);//设置能否输出。
		Log.setDisabled(!isEnableOutput());
		if (isEnableOutput() && outputToFile) {//判定能否输出且是否能输出到文件。如果能输出且输出到文件。 true&&false.
			File folder = new File(outputFolder);//路径名outputFolder 'output'
			if (!folder.exists()) {//如果文件不存在
				folder.mkdir();//创建一个目录。
			}

			File folder2 = new File(outputFolder + "/log");//创建一个File类对象，传入完整路径。
			if (!folder2.exists()) {//如果文件不存在
				folder2.mkdir();//创建一个目录
			}

			File file = new File(outputFolder + "/log/"
					+ getExperimentName(workload, vmAllocationPolicy, vmSelectionPolicy, parameter) + ".txt");//要操作 output/log/888_iqr_mmt_1.5.txt" 路径下的文件。
			file.createNewFile();//创建一个新文件。
			Log.setOutput(new FileOutputStream(file));//设置一个输出流。
		}
	}
	...
	/**
	 * Gets the experiment name. 获得实验名称。
	 * 
	 * @param args the args
...
//		System.out.println(experimentName.toString());
	...
	protected VmAllocationPolicy getVmAllocationPolicy(
			String vmAllocationPolicyName,//iqr
			String vmSelectionPolicyName,//mmt
			String parameterName/*1.5*/) {
