...
public class CloudletSchedulerDynamicWorkload extends CloudletSchedulerTimeShared {
	...
	public CloudletSchedulerDynamicWorkload(double mips, int numberOfPes) {
		super();
		setMips(mips);//设置处理能力
		setNumberOfPes(numberOfPes);//设置cpu处理单元的个数
		setTotalMips(getNumberOfPes() * getMips());//设置总的处理能力
		setUnderAllocatedMips(new HashMap<String, Double>());//创建一个正在分配处理能力列表。
		setCachePreviousTime(-1);//初始化上次缓存的时间
	}
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		double timeSpan = currentTime - getPreviousTime();//时间跨度，时间范围。
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			...
			if (rcl.getRemainingCloudletLength() == 0) { // finished: remove from the list
				...
				continue;
			} else { // not finish: estimate the finish time  如果没有完成：估计完成的时间
