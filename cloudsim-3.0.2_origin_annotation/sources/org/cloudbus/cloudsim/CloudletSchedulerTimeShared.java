...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public CloudletSchedulerTimeShared() {
		super();
		cloudletExecList = new ArrayList<ResCloudlet>();//事先创建好云任务执行列表
		cloudletPausedList = new ArrayList<ResCloudlet>();//事先创建好云任务暂停列表
		cloudletFinishedList = new ArrayList<ResCloudlet>();//事先创建好云任务完成列表
		currentCPUs = 0;//当前的cpu个数为0
	}
	...
	public boolean isFinishedCloudlets() {
		System.out.println("DanteFung,new add for test how many cloudlet had been completed in checkCloudletCompletion() which was in Datacenter processEvent(ev):" + getCloudletFinishedList().size());
