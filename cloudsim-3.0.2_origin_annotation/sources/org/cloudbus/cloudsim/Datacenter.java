...
public class Datacenter extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			case CloudSimTags.RESOURCE_CHARACTERISTICS:
				srcId = ((Integer) ev.getData()).intValue();//取出DatacenterBroker的ID
				sendNow(srcId, ev.getTag(), getCharacteristics());//直接發送消息給DatacenterBroker。這個消息攜帶了以下的數據：發送源DatacenterBroker的ID，DatacerBroker一開始發送請求時給事件打上的標籤CloudSimTags.RESOURCE_CHARACTERISTICS_REQUEST，DatacenterBroker所請求的數據中心的一些基礎硬件設施。
				break;
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (ack) {
			int[] data = new int[3];
			data[0] = getId();//数据中心的ID
			data[1] = vm.getId();//虚拟机的ID

			if (result) {//判断在主机上创建虚拟机是否成功。
				data[2] = CloudSimTags.TRUE;//标记为，成功。
			} else {
				data[2] = CloudSimTags.FALSE;//标记为，失败。
			}
			send(vm.getUserId(), 0.1, CloudSimTags.VM_CREATE_ACK, data);//给DatacenterBroker发送创建虚拟机的回馈信息（事件）。
		}
		...
		if (result) {//判断在主机上创建虚拟机是否成功。
			...
			vm.updateVmProcessing(CloudSim.clock(), getVmAllocationPolicy().getHost(vm).getVmScheduler()
					.getAllocatedMipsForVm(vm));//更新虚拟机上云任务的处理进程。
		}
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		Object tmp = ev.getData();//for (Map<String, Object> migrate : migrationMap) {...}
		...
		Vm vm = (Vm) migrate.get("vm");//通过虚拟机名取得对应的虚拟机对象。
		Host host = (Host) migrate.get("host");//通过主机名取得对应的目标主机对象。

		getVmAllocationPolicy().deallocateHostForVm(vm);//PowerVmAllocationPolicyAbstract.java  解除虚拟机与主机之间的绑定。
		host.removeMigratingInVm(vm);//删除虚拟机的迁移。到processVmMigrate(...)这个方法的时候，需要提前将虚拟机从目标主机的。
		...
		if (ack) {//如果执行迁移的后，需要给一个反馈信息回去给源实体：PowerDatacenter的话。
			int[] data = new int[3];
			data[0] = getId();
			data[1] = vm.getId();
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		updateCloudletProcessing();//先更新云任务的处理进程。

		try {
			// gets the Cloudlet object
			Cloudlet cl = (Cloudlet) ev.getData();

			// checks whether this Cloudlet has finished or not  先检查一下这个云任务是否已经完成了。
			if (cl.isFinished()) {
				String name = CloudSim.getEntityName(cl.getUserId());//通过云任务得到对应的用户的名称。
				Log.printLine(getName() + ": Warning - Cloudlet #" + cl.getCloudletId() + " owned by " + name
						+ " is already completed/finished.");
				Log.printLine("Therefore, it is not being executed again");
				Log.printLine();

				// NOTE: If a Cloudlet has finished, then it won't be processed.笔记：如果一个云任务已经处理完成，则不用处理。
				// So, if ack is required, this method sends back a result.因此，如果回应是必须的，这个方法返回一个结果。
				// If ack is not required, this method don't send back a result.如果这个回应不是必须的，这个方法不返回一个结果。
				// Hence, this might cause CloudSim to be hanged since waiting 因此，这可能导致CloudSim因为等待这个云任务返回而被悬挂。
				// for this Cloudlet back.
				if (ack) {
					int[] data = new int[3];
					data[0] = getId();//Datacenter的ID
					data[1] = cl.getCloudletId();//云任务的ID
					data[2] = CloudSimTags.FALSE;

					// unique tag = operation tag  独一无二的标签 = 操作标签
					int tag = CloudSimTags.CLOUDLET_SUBMIT_ACK;//
					sendNow(cl.getUserId(), tag, data);//发送一个事件，参数：云任务用户ID，CloudSimTags.CLOUDLET_SUBMIT_ACK，
				}

				sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);//发送反馈信息给DatacenterBroker，云任务处理完成。
			...
			cl.setResourceParameter(getId(), getCharacteristics().getCostPerSecond(), getCharacteristics()
					.getCostPerBw());//设置云资源参数。

			int userId = cl.getUserId();//用户ID
			int vmId = cl.getVmId();//虚拟机ID

			// time to transfer the files  传输文件的时间
			double fileTransferTime = predictFileTransferTime(cl.getRequiredFiles());//预计文件传输的时间。
			...
			// if this cloudlet is in the exec queue 如果这个云任务是执行队列里面的
			if (estimatedFinishTime > 0.0 && !Double.isInfinite(estimatedFinishTime)) {//预计完成的时>0.0且预计要完成时间是否不是无限大值，如果是的话执行以下代码。
				estimatedFinishTime += fileTransferTime;
				send(getId(), estimatedFinishTime, CloudSimTags.VM_DATACENTER_EVENT);/*参数：目标实体为Datacenter自己的ID，预计要完成的时间，一个内部事件在PowerDatacenter中生成。
				 																     *意味着：Datacenter自己给自己发送事件。
				 																     */
			}
			...
			if (ack) {
				int[] data = new int[3];
				data[0] = getId();//Datacenter的ID
				data[1] = cl.getCloudletId();//云任务自己的ID
				data[2] = CloudSimTags.TRUE;//

				// unique tag = operation tag
				int tag = CloudSimTags.CLOUDLET_SUBMIT_ACK;
				sendNow(cl.getUserId(), tag, data);//cl.getUserId（）赋值给dest，即目标实体的ID。本类Datacenter的ID做为发送源的ID。
			}
		} catch (ClassCastException c) {
		...
		checkCloudletCompletion();//检查云任务是否
	}
	...
	protected void updateCloudletProcessing() {
		...
		if (CloudSim.clock() < 0.111 || CloudSim.clock() > getLastProcessTime() + 0.1) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				// inform VMs to update processing 通知所有的虚拟机更新云任务的处理进程。
	...
	protected void checkCloudletCompletion() {
		List<? extends Host> list = getVmAllocationPolicy().getHostList();//取得主机列表
		for (int i = 0; i < list.size(); i++) {
			Host host = list.get(i);//迭代出列表里的所有主机。
			for (Vm vm : host.getVmList()) {//迭代出主机上的每台虚拟机
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {	//查看每台虚拟机上云任务完成列表是否>0，即是否列表里存有已经完成的云任务。			
					Cloudlet cl = vm.getCloudletScheduler().getNextFinishedCloudlet();//从云任务完成列表中取出完成的云任务。
					if (cl != null) {//如果云任务不为空。
						sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);//发送一个云任务完成反馈信息给PowerDatacenterBroker
					}
				}
			}
		}
