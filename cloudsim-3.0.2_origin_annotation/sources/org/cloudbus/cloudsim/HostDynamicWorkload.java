...
public class HostDynamicWorkload extends Host {
	...
	public double updateVmsProcessing(double currentTime) {
		double smallerTime = super.updateVmsProcessing(currentTime);//先调用父类Host的updateVmsProcessing（）方法，在虚拟机上更新任务运行处理。
		setPreviousUtilizationMips(getUtilizationMips());//设置上一次Cpu利用率。
		setUtilizationMips(0);//将Cpu利用率设置为0
		double hostTotalRequestedMips = 0;

		for (Vm vm : getVmList()) {
			getVmScheduler().deallocatePesForVm(vm);//将分配给虚拟机的cpu处理能力pe回收。
		}

		for (Vm vm : getVmList()) {
			getVmScheduler().allocatePesForVm(vm, vm.getCurrentRequestedMips());//分配cpu处理能力pe给虚拟机。
		}

		for (Vm vm : getVmList()) {
			double totalRequestedMips = vm.getCurrentRequestedTotalMips();//计算出虚拟机请求的总的cpu处理能力mips。
			double totalAllocatedMips = getVmScheduler().getTotalAllocatedMipsForVm(vm);//计算出总共分配给虚拟机的cpu处理能力mips。
			...
			if (!Log.isDisabled()) {
				Log.formatLine(
						...
						CloudSim.clock(),
						totalAllocatedMips,//当前的虚拟机的总的被分配的处理能力
						totalRequestedMips,//当前的虚拟机总请求的处理能力。
						vm.getMips(),//
						totalRequestedMips / vm.getMips() * 100);//

				List<Pe> pes = getVmScheduler().getPesAllocatedForVM(vm);//取出分配给这台虚拟机的cpu处理能力pe列表。
			...
			if (getVmsMigratingIn().contains(vm)) {
				...
				}//CloudSim.clock：主机#ID正在分配处理能力给虚拟机#ID：处理能力的值。

				vm.addStateHistoryEntry(//将这台虚拟机当前的重要信息，存入历史状态条目列表里面。
						currentTime,
						totalAllocatedMips,
						totalRequestedMips,
						(vm.isInMigration() && !getVmsMigratingIn().contains(vm)));

				if (vm.isInMigration()) {//判断虚拟机是否在迁移。
					Log.formatLine(
							"%.2f: [Host #" + getId() + "] VM #" + vm.getId() + " is in migration",
							CloudSim.clock());
					totalAllocatedMips /= 0.9; // performance degradation due to migration - 10% 由于迁移性能下降10%
				}
			}

			setUtilizationMips(getUtilizationMips() + totalAllocatedMips);//设置这台主机的cpu利用率。
			hostTotalRequestedMips += totalRequestedMips;//所有虚拟机请求的处理能力作为主机总共被请求的所有处理能力。
		}

		addStateHistoryEntry(//将这台主机当前的重要信息，存入历史状态条目列表里面。
				currentTime,
				getUtilizationMips(),
				hostTotalRequestedMips,
				(getUtilizationMips() > 0));

		return smallerTime;//返回各次虚拟机返回云任务预计完成的时间中，耗时最少的。
	}
