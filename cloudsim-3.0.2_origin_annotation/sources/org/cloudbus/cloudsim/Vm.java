...
public class Vm {
	...
	public List<Double> getCurrentRequestedMips() {
		...
		if (isBeingInstantiated()) {//检查是否正在被实例化。
	...
	public void addStateHistoryEntry(
		...
		if (!getStateHistory().isEmpty()) {
			VmStateHistoryEntry previousState = getStateHistory().get(getStateHistory().size() - 1);//取出上一次虚拟机的状态。
			if (previousState.getTime() == time) {//如果上次虚拟机的状态的时刻等于当前时间。
				getStateHistory().set(getStateHistory().size() - 1, newState);//替换指定位置的元素。
				return;
			}
		}
