...
public class VmSchedulerTimeSharedOverSubscription extends VmSchedulerTimeShared {
	...
	protected boolean allocatePesForVm(String vmUid, List<Double> mipsShareRequested) {
		...
		for (Double mips : mipsShareRequested) {
			if (mips > peMips) {//如果虚拟机每个pe所需要的mips>主机的pe的mips的话
				mipsShareRequestedCapped.add(peMips);//
				totalRequestedMips += peMips;//累加主机分配给虚拟机的mips
			} else {//如果是小于等于的话，
				mipsShareRequestedCapped.add(mips);
				totalRequestedMips += mips;//累计虚拟机自己pe的mips。
			}
		}

		getMipsMapRequested().put(vmUid, mipsShareRequested);//参数：虚拟机ID，存储了虚拟机请求的mips列表。（mips-pe，虚拟机有多少个pe就有多少个mips，以列表的形式存储）  意味着：记录具体哪台虚拟机向主机请求了多少的处理能力。
		setPesInUse(getPesInUse() + mipsShareRequested.size());//更新设置正在使用的pe数。
		...
		if (getAvailableMips() >= totalRequestedMips) {//如果可用的mips大于等于虚拟机请求的mips
			List<Double> mipsShareAllocated = new ArrayList<Double>();
			for (Double mipsRequested : mipsShareRequestedCapped) {
				if (getVmsMigratingOut().contains(vmUid)) {
					// performance degradation due to migration = 10% MIPS  由于迁移= 10% MIPS性能下降
					mipsRequested *= 0.9;
