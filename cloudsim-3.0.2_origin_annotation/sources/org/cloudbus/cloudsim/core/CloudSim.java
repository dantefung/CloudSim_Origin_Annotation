...
public class CloudSim {
	...
	public static void init(int numUser, Calendar cal, boolean traceFlag) {
		try {
			...
			// create a GIS object  创建云信息服务对象
			cis = new CloudInformationService("CloudInformationService");//通过构造方法，将云信息服务对象自己放入到CloudSim中的entity列表中。
	...
	// The predicates used in entity wait methods 这个述词用于实体的等待的方法。
	/** The wait predicates. */
	...
	public static boolean runClockTick() {
		...
		if (future.size() > 0) {
			System.out.println("此时，future队列中有：" + future.size() + "个事件。");
			...
			// Check if next events are at same time...  检查队列后面的事件是否是同时提交的。
			boolean trymore = it.hasNext();
			while (trymore) {
				SimEvent next = it.next();
				if (next.eventTime() == first.eventTime()) {//判断后面的事件，是否是跟第一个事件是同时提交的。
					processEvent(next);
					toRemove.add(next);
					trymore = it.hasNext();
				} else {
	 ...
	 * 检查是否有一些给一个特定的实体处理事件存在于等待事件队列里面。
	 * 
	 * @param d the d
	 * @param p the p
	 * @return the int
	...
	public static SimEvent select(int src, Predicate p) {
		...
		while (iterator.hasNext()) {
			ev = iterator.next();
			if (ev.getDestination() == src && p.match(ev)) {//判断目标实体的id是否等于本实体的id。
				iterator.remove();//将该事件从deferred队列中移除。
				break;
			}
		}
		return ev;
	...
	private static void processEvent(SimEvent e) {
		...
		clock = e.eventTime();//更新时间，之后事件中携带的时间，是有当前的模拟时间，是下一个运行在一个特定主机的某个虚拟机上的任务的预计完成时间。各次主机返回完成的最少时间是下一次内部事件时间的标准。

		// Ok now process it
		switch (e.getType()) {//取出事件的内部类型。
			case SimEvent.ENULL:
			...
			case SimEvent.SEND:
				...
				dest = e.getDestination();//取出该事件指定的目的实体ID。
				if (dest < 0) {
					throw new IllegalArgumentException("Attempt to send to a null entity detected.");//检测到试图发送给一个空实体。
				} else {//如果目的实体是存在的。
					int tag = e.getTag();//取出事件的外部标签。
					dest_ent = entities.get(dest);//根据事件指定的目的实体ID从实体列表中取出该目的实体。
					if (dest_ent.getState() == SimEntity.WAITING) {//查看该实体是否在一直在等待。
						Integer destObj = Integer.valueOf(dest);//将int类型数据转成Integer对象。
						Predicate p = waitPredicates.get(destObj);//从Map集合中取出  等待实体  所对应的predicate。
						if ((p == null) || (tag == 9999) || (p.match(e))) {
							dest_ent.setEventBuffer((SimEvent) e.clone());
							dest_ent.setState(SimEntity.RUNNABLE);//将正在处于等待状体的实体设置为运行状态。
							waitPredicates.remove(destObj);//从map集合中删除掉该等待状态的实体。
						} else {
							deferred.addEvent(e);
						}
					} else {//如果该目的实体不是处于一直等待的状态。
						deferred.addEvent(e);//将该事件放入到deffered队列里面。
					}
				}
				break;
