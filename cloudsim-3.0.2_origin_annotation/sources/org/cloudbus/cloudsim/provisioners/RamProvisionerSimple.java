...
public class RamProvisionerSimple extends RamProvisioner {
	...
	public void deallocateRamForVm(Vm vm) {
		if (getRamTable().containsKey(vm.getUid())) {//如果内存表格（实质是Map集合）中存在userid+vmid
			int amountFreed = getRamTable().remove(vm.getUid());//解除虚拟机的userid+id与之前分配给它的内存的绑定，返回值为之前分配给这台虚拟机的内存。
			setAvailableRam(getAvailableRam() + amountFreed);//旧主机重置可用的内存。
			vm.setCurrentAllocatedRam(0);//将当前分配给虚拟机的内存置0.
		}
	}
